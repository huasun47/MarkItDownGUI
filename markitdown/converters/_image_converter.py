...
class ImageConverter(DocumentConverter):
    ...
    def convert(
        ...
        **kwargs: Any,  # Options to pass to the converter
        ...
        # Try OCR
        try:
            if 'Image' in globals() and 'pytesseract' in globals():
                cur_pos = file_stream.tell()
                try:
                    image = Image.open(file_stream)
                    # Try to use Chinese and English
                    text = pytesseract.image_to_string(image, lang='chi_sim+eng')
                    if text.strip():
                        md_content += "\n# Text Content:\n" + text.strip() + "\n"
                except pytesseract.TesseractNotFoundError:
                     md_content += "\n> [!WARNING]\n> Tesseract OCR not found. Please ensure Tesseract-OCR is installed and added to PATH.\n"
